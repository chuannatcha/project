...
const unsigned long MaxEnSavingTime = 10000;
const unsigned long MaxDisSavingTime = 5000;
const unsigned long MaxEnSecurityTime = 5000;
const unsigned long MaxDisSecurityTime = 5000;
const unsigned long SecurityModeStartTime = 5000;
const char SwitchModePin = 18;
...
void Enable_Security()
{
  if (Tstatus[M1] || Tstatus[M2] || Tstatus[M3])
  {
    if ((millis() - EnSecurityBeginCount) > MaxEnSecurityTime)
    {
      ...
      delay(1000);
      Serial.print("AT+CMGS=\"0842216218\"\r");
      delay(1000);
      Serial.print("Found Theif in the RMUTL ELEC Computer Room\r");   //The text for the message
      delay(1000);
      Serial.write(26);  //Equivalent to sending Ctrl+Z 
      
      Serial2.println("LA-R-1#110");
      SecurityModeRunning = true;
      Serial.println("Found theif");
      digitalWrite(SavingMoniPin, LOW);
      digitalWrite(SavingActPin, LOW);
      digitalWrite(SecurityMoniPin, LOW);
...
void SwitchMode()
{
  if (!DeviceMode)
  {
    ...
    Serial2.println("RA-R-0#113");//Cancel All Remote Command that operated
    DeviceMode = true;
